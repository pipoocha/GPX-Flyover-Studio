...
class DirectorCamera:
    ...
    def __init__(self, path_coords):
        ...
        self.local_fit_distance_scale = float(
            getattr(config, "CAMERA_LOCAL_FIT_DISTANCE_SCALE", 0.52)
        )
        self.local_fit_height_scale = float(
            getattr(config, "CAMERA_LOCAL_FIT_HEIGHT_SCALE", 0.30)
        )
        self.local_fit_min_distance = float(
            getattr(config, "CAMERA_LOCAL_FIT_MIN_DISTANCE", 1300.0)
        )
        self.local_fit_max_distance = float(
            getattr(config, "CAMERA_LOCAL_FIT_MAX_DISTANCE", 4200.0)
        )
        self.local_fit_min_height = float(
            getattr(config, "CAMERA_LOCAL_FIT_MIN_HEIGHT", 750.0)
        )
        self.local_fit_max_height = float(
            getattr(config, "CAMERA_LOCAL_FIT_MAX_HEIGHT", 2600.0)
        )
