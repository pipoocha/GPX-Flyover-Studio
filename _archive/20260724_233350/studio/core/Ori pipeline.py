...
class FlyoverPipeline:
    ...
    def load_satellite_texture(self):
        if not bool(getattr(config, "USE_SATELLITE", False)):
            self.satellite_texture = None
            return

        zoom = int(getattr(config, "SATELLITE_ZOOM", 14))
        flip_vertical = bool(
        ...
        if not satellite.exists():
            raise FileNotFoundError(
                ...
                "Lance d'abord :\n"
                f'python satellite_downloader.py '
                f'"{self.project.gpx_file}" --zoom {zoom}'
            )
