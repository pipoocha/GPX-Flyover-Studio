...
class PreviewPlayer:
    ...
    def build_track_mesh(self, visible_path):
        render_mode = str(getattr(config, "TRACK_RENDER_MODE", "line")).lower()
        if render_mode == "line":
            return pv.lines_from_points(visible_path)
        return Track(visible_path, radius=config.TRACK_RADIUS, sides=config.TRACK_SIDES).to_mesh()

    def add_track_actor(self, visible_path):
        mesh = self.build_track_mesh(visible_path)
        render_mode = str(getattr(config, "TRACK_RENDER_MODE", "line")).lower()
        if render_mode == "line":
            line_width = float(getattr(config, "TRACK_LINE_WIDTH", 2.0))
        ...
        return self.scene.add_mesh(mesh, color="#FC4C02", smooth_shading=True)

    def update_track(self, force=False):
        update_every = max(1, int(getattr(config, "TRACE_UPDATE_EVERY", 6)))
        if (not force and self.last_track_frame >= 0 and
                abs(self.frame_index - self.last_track_frame) < update_every):
            return
        ...
        if self.track_actor is not None:
            try:
                plotter.remove_actor(self.track_actor, render=False)
            except Exception:
                pass
        self.track_actor = self.add_track_actor(visible_path)
    ...
    def print_header(self):
        print()
        print("===================================")
        print("PREVIEW DIRECTOR RAPIDE")
        print("-----------------------------------")
        print("Caméra      :", getattr(config, "CAMERA_MODE", "flyover"))
        print("Orientation :", getattr(config, "CAMERA_ORIENTATION_MODE", "route"))
        print("Trace       :", getattr(config, "TRACK_RENDER_MODE", "line"))
        print("Largeur     :", getattr(config, "TRACK_LINE_WIDTH", 2.0), "px")
        print("-----------------------------------")
    ...
    def play(self):
        print("TRACK_RENDER_MODE =", config.TRACK_RENDER_MODE)
        print("TRACK_LINE_WIDTH  =", getattr(config, "TRACK_LINE_WIDTH", "ABSENT"))
        self.print_header()
        plotter = self.scene.plotter
        ...
        while not self.stopped:
            ...
            if not self.paused:
                ...
                if (self.frame_index % display_interval == 0 and
                        self.frame_index != last_display):
                    print(
